 ...
 */
async function handleDataProcessingResult({ topic, pair, partition } = dataPackage) {

    // we must use the base message handler
    const model = handleMessage(dataPackage)

    // Log the message header details
    logger.debug(`[handleDataProcessingResult] [debug] Ready message model`);

    try {

        if (model) {
            try {
                logger.debug(`[handleDataProcessingResult] [debug] message model is valid: ${JSON.stringify(model)}`);
                const { timestamp } = model
                const { key } = pair
                logger.notice(`[handleDataProcessingResult] [notice] key: ${key}, timestamp: ${timestamp}`)
                // Do something
            } catch (error) {
                logger.warn(`[handleDataProcessingResult] [warn]  message model is no valid. error: ${error.message}, value: ${JSON.stringify(model)}`);
            }
        }

    } catch (error) {
        // Log any error during the processing of the message
        logger.error(`[handleDataProcessingResult] [error] Error message model - topic: ${topic}, partition: ${partition}, error: ${error.message}`);
    }
}

module.exports = { handleDataProcessingResult };
